...
Outputs:
  1. aego_metrics_summary.csv        (Summary table with all calculated
  metrics)
  2. aego_detailed_experiments.csv   (Granular per-experiment EN, t_exp,
  t_exp_rtt, rates)
  3. aego_metrics_comparison.png     (Line plots with 3 rate representations)
  4. aego_metrics_comparison_seq.png (Bar charts with 3 rate representations
  per pgen)
  5. aego_attempts_duration.png      (Average N attempts and 2 duration
  representations)
"""
...
T_ATTEMPT_DEFAULT_NS = (
    400000.0  # 400 us (Duración de intento por defecto si no hay RTT)
)
...
def compute_w_experimental(
    t_vuelta_ns,
    tau_ns=TAU_RELATIVISTIC_NS,
    w0=W0_INITIAL,
    t_mem_ns=T_MEMORY_NS,
):
...
def find_sender_csv_files(dir_path, selected_indices=None):
  ...
  for idx_pos, filepath in enumerate(sender_files, start=1):
    ...
    num_match = re.search(
        r'sender_timing_([0-9]+)\.csv', filename, re.IGNORECASE
    )
    run_num = int(num_match.group(1)) if num_match else None

    if (run_num is not None and run_num in selected_indices) or (
        idx_pos in selected_indices
    ):
...
def process_sender_data(df):
  ...
  for _, row in df.iterrows():
    ...
    if row.get('success_bit', 0) == 1:
      ...
      if n_attempts > 1:
        t_attempt_ns = (
            current_attempts[-1]['emit_ts_ns'] - start_ts_ns
        ) / (n_attempts - 1)
      else:
        last_row = current_attempts[-1]
        if (
            'rtt_ns' in last_row
            and pd.notnull(last_row['rtt_ns'])
            and last_row['rtt_ns'] > 0
        ):
      ...
      tv_vals_ns = [
          r['rtt_ns'] - r['e2r_ns']
          for r in current_attempts
          if 'rtt_ns' in r
          and 'e2r_ns' in r
          and pd.notnull(r['rtt_ns'])
          and pd.notnull(r['e2r_ns'])
      ]
...
def analyze_pgen_dir(dir_path, filter_percentile=95, selected_indices=None):
  ...
  df_exp['t_exp_theoretical_ms'] = df_exp['n_attempts'] * (
      T_ATTEMPT_DEFAULT_NS / 1e6
  )
  ...
  diff_rate_tp_pct = (
      (rate_real_throughput - rate_fixed_throughput) / rate_fixed_throughput
  ) * 100.0
  diff_rate_att_pct = (
      (rate_real_attempt - rate_fixed_attempt) / rate_fixed_attempt
  ) * 100.0
...
def print_t_vuelta_summary(df_res, selected_indices=None):
  indices_label = (
      f'{sorted(list(selected_indices))}' if selected_indices else 'ALL'
  )
  print('\n' + '=' * 90)
  print(
      f' MEAN t_vuelta & DIFFERENCE SUMMARY PER pgen (CSV Index:'
      f' {indices_label}) '
  )
  print('=' * 90)
...
def plot_overlay_comparison(df_res, output_plot):
  ...
  axes[0].plot(
      df_res['pgen'],
      df_res['Rate_real_throughput'],
      'd-',
      color='#d62728',
      linewidth=2,
      label='Rate Empirical (Emit TS)',
  )
  axes[0].plot(
      df_res['pgen'],
      df_res['Rate_rtt_throughput'],
      's-',
      color='#2ca02c',
      linewidth=2,
      label='Rate Empirical (RTT Sum)',
  )
  ...
  axes[0].set_title(
      'Throughput Rate Comparison', fontsize=12, fontweight='bold'
  )
  ...
  axes[1].set_title(
      'Attempt-Normalized Rate Comparison', fontsize=12, fontweight='bold'
  )
...
def plot_sequential_comparison(df_res, output_plot, selected_indices=None):
  ...
  axes[0].bar(
      ...
      df_res['Rate_rtt_throughput'],
      width,
      label='Rate Real (RTT Sum)',
      color='#2ca02c',
      alpha=0.85,
  )
  axes[0].set_title(
      'Throughput Rate [e-bits/s]', loc='left', fontsize=11, fontweight='bold'
  )
  ...
  axes[1].set_title(
      'Attempt-Normalized vs. Burst Rate',
      loc='left',
      fontsize=11,
      fontweight='bold',
  )
...
def plot_attempts_and_duration(df_res, output_plot):
  ...
  axes[0].set_title(
      'Average Attempts per Experiment', fontsize=12, fontweight='bold'
  )
  axes[0].grid(True, linestyle='--', alpha=0.6)
  ...
  axes[1].plot(
      df_res['pgen'],
      df_res['t_exp_mean_ms'],
      's-',
      color='#ff7f0e',
      linewidth=2,
      markersize=6,
      label=r'$\langle t_{exp} \rangle$ (Emit TS)',
  )
  axes[1].plot(
      df_res['pgen'],
      df_res['t_exp_rtt_mean_ms'],
      'd-',
      color='#2ca02c',
      linewidth=2,
      markersize=6,
      label=r'$\langle t_{exp} \rangle$ (RTT Sum)',
  )
  axes[1].set_xlabel(r'Generation Probability ($p_{gen}$)', fontsize=11)
  axes[1].set_ylabel(
      r'Average Duration $\langle t_{exp} \rangle$ [ms]', fontsize=11
  )
  axes[1].set_title(
      'Average Experiment Duration Comparison', fontsize=12, fontweight='bold'
  )
  axes[1].grid(True, linestyle='--', alpha=0.6)
  axes[1].legend()
...
def generate_all_plots(df_res, selected_indices=None):
  ...
  plot_sequential_comparison(
      df_res,
      'aego_metrics_comparison_seq.png',
...
def main():
  parser = argparse.ArgumentParser(
      description=(
          'AEGO Entanglement Analysis (Fixed vs Empirical vs RTT-Sum)'
      )
  )
  parser.add_argument('--dir', default='.', help='Data directory')
  parser.add_argument(
      '--out', default='aego_metrics_summary.csv', help='Summary output CSV'
  )
  parser.add_argument(
      '--out-detailed',
      default='aego_detailed_experiments.csv',
      help='Granular per-experiment output CSV',
  )
  parser.add_argument(
      '--filter-p',
      type=float,
      default=99.0,
      help='Upper percentile cutoff for anomalies (default: 95)',
  )
  parser.add_argument(
      '--csv-indices',
      nargs='+',
      help='Indices of sender CSV files (e.g., --csv-indices 4 5 6)',
  )
  ...
  if selected_csv_indices:
    print(
        '[*] Filtering sender files matching numbers/indices:'
        f' {sorted(list(selected_csv_indices))}'
    )

  subdirs = [
      os.path.join(args.dir, d)
      for d in os.listdir(args.dir)
      if 'pgen' in d and os.path.isdir(os.path.join(args.dir, d))
  ]
  ...
  for d in sorted(subdirs):
    summary, df_clean = analyze_pgen_dir(
        d, args.filter_p, selected_csv_indices
    )
  ...
  if summary_results:
    df_res = (
        pd.DataFrame(summary_results).sort_values(by='pgen').reset_index(drop=True)
    )
    ...
    cols_to_show = [
        ...
        'Rate_real_indiv_mean',  # <R_indiv> (Burst Rate)
        'w_exp_real',
        'diff_w_%',
    ]
