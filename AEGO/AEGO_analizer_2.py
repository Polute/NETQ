...
1. t_gen vs pgen and N_gen vs pgen (Mean and Standard Deviation).
2. Rate Heatmap (Eq. 8) as a function of pgen (x-axis) and Tcoh (y-axis).
3. Heatmap of Raw E_N per attempt (normalized by total N_attempts).
4. Heatmap of Raw E_N per successful generation (normalized by M_success).
5. Summary ASCII table printed to stdout terminal and saved as .txt file.
...
def process_sender_data_pure_empirical(df):
    """Processes attempts directly using NumPy arrays to avoid iterrows()."""
    df_valid = df.dropna(subset=['emit_ts_ns', 'rtt_ns', 'e2r_ns'])
    df_valid = df_valid[(df_valid['rtt_ns'] > 0) & (df_valid['e2r_ns'] > 0)].sort_values('emit_ts_ns')

    if df_valid.empty:
        return pd.DataFrame()

    emit_ts = df_valid['emit_ts_ns'].values
    rtt = df_valid['rtt_ns'].values
    e2r = df_valid['e2r_ns'].values
    success = df_valid['success_bit'].values if 'success_bit' in df_valid else np.zeros(len(df_valid))
    ...
    for i in range(len(df_valid)):
        if success[i] == 1:
            n_attempts = i - idx_start + 1
            start_ts = emit_ts[idx_start]
            
            if n_attempts > 1:
                t_attempt_ns = (emit_ts[i] - start_ts) / (n_attempts - 1)
            else:
                t_attempt_ns = float(rtt[i])

            end_ts = emit_ts[i] + t_attempt_ns
            t_gen_s = (end_ts - start_ts) / 1e9

            e2r_mean_ns = float(np.mean(e2r[idx_start:i+1]))
            rtt_mean_ns = float(np.mean(rtt[idx_start:i+1]))
...
def plot_tgen_vs_pgen(tgen_means_ms, tgen_stds_ms, sorted_pgens, mode_label, w0_val, output_dir=OUTPUT_DIR):
    ...
    file_path = os.path.join(output_dir, f"plot_1_tgen_vs_pgen.png")
...
def plot_ngen_vs_pgen(ngen_means, ngen_stds, sorted_pgens, mode_label, w0_val, output_dir=OUTPUT_DIR):
    ...
    file_path = os.path.join(output_dir, f"plot_1b_ngen_vs_pgen.png")
    plt.figure(figsize=(8, 5))
    pgens_str = [f"{p:.1f}" for p in sorted_pgens]
    ...
    plt.title(f'Attempts needed to generate $N_{{gen}}$ vs $p_{{gen}}$ (Mean $\pm \sigma$)', fontsize=12, fontweight='bold')
    plt.grid(True, linestyle='--', alpha=0.5)
    plt.legend(loc='upper right', fontsize=10)
...
def plot_rate_heatmap(rate_matrix, sorted_pgens, mode_label, w0_val, output_dir=OUTPUT_DIR, filename_suffix="one_way"):
    ...
    file_path = os.path.join(output_dir, f"plot_2_heatmap_Rate_vs_pgen_Tcoh_{filename_suffix}.png")
...
def plot_raw_en_heatmap(df_pivot, mode_label, output_dir=OUTPUT_DIR, filename_suffix="one_way"):
    """Plots raw Logarithmic Negativity E_N normalized by total attempts (N_attempts)."""
    file_path = os.path.join(output_dir, f"plot_3_raw_EN_per_attempt_heatmap_{filename_suffix}.png")
    ...
    ax.set_title(
        f"Heatmap of Raw $E_N$ per Attempt vs $p_{{gen}}$ and $T_{{coh}}$ ({mode_label})",
...
def plot_en_per_success_heatmap(df_pivot, mode_label, output_dir=OUTPUT_DIR, filename_suffix="one_way"):
    """Plots raw Logarithmic Negativity E_N per successful generation (M_success)."""
    file_path = os.path.join(output_dir, f"plot_4_EN_per_success_heatmap_{filename_suffix}.png")
    fig, ax = plt.subplots(figsize=(10, 6))
    ...
    ax.set_ylabel('Coherence Time ($T_{\mathrm{coh}}$)', fontsize=11, labelpad=10)
    ax.set_title(
        f"Heatmap of Raw $E_N$ per Successful Generation vs $p_{{gen}}$ and $T_{{coh}}$ ({mode_label})",
        fontsize=11, fontweight="bold", pad=12
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float, 
        default=100.0, 
        help="Upper percentile threshold to filter t_gen_s outliers (default: 100.0)"
    )
    parser.add_argument(
        "--w0",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=2000.0,
        help="Microsecond (µs) threshold to alert on extreme t_gen values (default: 2000.0)"
    )
    args = parser.parse_args()

    os.makedirs(OUTPUT_DIR, exist_ok=True)

    # 1. Cargar y procesar archivos CSV una sola vez
    subdirs = [os.path.join('.', d) for d in os.listdir('.') if 'pgen' in d and os.path.isdir(os.path.join('.', d))]
    pgen_data_dict = {}

    print("\n[*] Loading and parsing empirical CSV files...")
    for d in sorted(subdirs):
        ...
        for f in csv_files:
            if os.path.getsize(f) > 0:
                ...
                df_proc = process_sender_data_pure_empirical(df_single)
    ...
    # 2. Computar matrices y gráficos reutilizando los DataFrames
    for mode_key, mode_label in modes:
        ...
        en_success_means_matrix = np.zeros((num_tcoh, num_pgen))
        ...
        for col_idx, pgen in enumerate(sorted_pgens):
            ...
            t_roundtrip_ns = df_gen['e2r_ns'].values if mode_key == 'one_way' else df_gen['rtt_ns'].values
            ...
            for row_idx, t_coh_ns in enumerate(TCOH_VALUES_NS):
                ...
                # Plot 3: E_N promilado por total de intentos
                raw_en_means_matrix[row_idx, col_idx] = np.sum(en_vals_raw) / np.sum(n_gen_vals)

                # Plot 4: E_N promedio por cada éxito
                en_success_means_matrix[row_idx, col_idx] = np.mean(en_vals_raw)

                # Rate matrix (con truncamiento)
                en_vals = en_vals_raw.copy()
                en_vals[en_vals < 0.3] = 0.0 
                rate_matrix[row_idx, col_idx] = np.mean(en_vals / t_gen_s)

        pgens_str = [f"{p:.1f}" for p in sorted_pgens]
        df_raw_pivot = pd.DataFrame(raw_en_means_matrix, index=TCOH_LABELS, columns=pgens_str)
        df_success_pivot = pd.DataFrame(en_success_means_matrix, index=TCOH_LABELS, columns=pgens_str)
        ...
        plot_en_per_success_heatmap(df_success_pivot, mode_label, filename_suffix=mode_key)
