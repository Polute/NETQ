...
def main():
    parser = argparse.ArgumentParser(description="Standalone Unicast PTP synchronization daemon for DEOS.")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--master", action="store_true", help="Act as PTP Grandmaster (R1).")
    group.add_argument("--slave", type=str, help="IP of the PTP Master (for R2, Alice, and Bob).")
    ...
    ptp4l_cmd = ["sudo", "ptp4l", "-i", args.interface, "-f", config_file, "-m"]
    if not args.master:
        ptp4l_cmd.append("-s")
    ...
    print("[PTP Slave] Synchronizing system clock... Waiting for convergence.")
    ...
    while True:
        ...
        if match:
            ...
            if current_offset_abs <= args.target_offset and not has_reached_target:
                ...
                msg = f"[PTP SUCCESS] Clock Synchronized! P2P Precision: {jitter_p2p} ns."
                print(f"\n\n{msg}")
                print("[*] Keeping synchronization active in background. You may launch DEOS.")
